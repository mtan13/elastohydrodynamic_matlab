    ...
    while criteria==false% The inner loop
       ...
       % peguess(1) = peguess(2);
        ...
        if max(abs(hcalc-hguess1))< 0.5E-10
            ...
            RMax = 0.1*max(Rs);
            YAxis = 1.1* h0';
            if rem(runs,10)<1  % plot real profile, guess, errors every 10 trys
                subplot(1,3,3)
                plot(Rs,allhcalc(runs,:)-allhguess1(runs,:))
                title(num2str(ratio));
                axis([0 RMax -1e-9 1e-9])
                drawnow
                subplot(1,3,2)
                hold on
                plot(Rs,hguess1,'red')
            ...
            if runs > 10000  % this if statement makes stop the program when reach certain amount of trys each loop
                break
            end
